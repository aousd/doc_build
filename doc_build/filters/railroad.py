...
class AlternatingSequence(DiagramMultiContainer):
    ...
    def textDiagram(self) -> TextDiagram:
        ...
        separator += [
            (" " * (leftWidth - 1)) + " " + cross_diag + " " + (" " * (rightWidth - 2))
        ]
        separator += [
            (line * (leftWidth - 1))
            + corner_bot_right
        ...
        diagramTD = diagramTD.appendBelow(
            secondTD, separator, moveEntry=True, moveExit=True
        )
        leftLines += [corner_top_left + line]
        ...
        diagramTD = diagramTD.alter(
            entry=firstTD.height + (len(separator) // 2),
            exit=firstTD.height + (len(separator) // 2),
        )
        leftTD = TextDiagram(
            firstTD.height + (len(separator) // 2),
            firstTD.height + (len(separator) // 2),
            leftLines,
        )
        rightTD = TextDiagram(
...
class Choice(DiagramMultiContainer):
    ...
    def format(self, x: float, y: float, width: float) -> Choice:
        ...
        for i in range(self.default + 1, len(self.items)):
            ...
            upperItem = self.items[i - 1]
            distanceFromY += (
                upperItem.height + upperItem.down + self.separators[i - 1] + item.up
            )
    ...
    def textDiagram(self) -> TextDiagram:
        ...
        for itemNum, itemTD in enumerate(itemTDs):
            ...
            separator = (
                [
                    line_vertical
                    + (" " * (TextDiagram._maxWidth(diagramTD, itemTD) - 2))
                    + line_vertical
                ]
                if hasSeparator
                else []
            ...
            diagramTD = diagramTD.appendBelow(
                itemTD, separator, moveEntry=moveEntry, moveExit=moveExit
            )
        return diagramTD
...
class HorizontalChoice(DiagramMultiContainer):
    ...
    def textDiagram(self) -> TextDiagram:
        ...
        baselineToSUIL = (
            max(
                [
                    itemTD.height - min(itemTD.entry, itemTD.exit)
                    for itemTD in itemTDs[1:]
                ]
            )
            - 1
        )
        ...
        for itemNum, itemTD in enumerate(itemTDs):
            ...
            if itemNum > 0:
                ...
                lines = []
                lines += [" " * itemTD.width] * (
                    baselineToSUIL - (itemTD.height - itemTD.entry) + 1
                )
                lines += [line * itemTD.width]
            ...
            if itemNum < len(itemTDs) - 1:
                ...
                lines += [line + roundcorner_top_right]
                lines += [" " + line_vertical] * (
                    baselineToSUIL - (diagramTD.exit - diagramTD.entry)
                )
...
class OneOrMore(DiagramItem):
    ...
    def textDiagram(self) -> TextDiagram:
        ...
        leftLines += [repeat_left + " "] * (
            (itemTD.height - itemTD.entry) + repeatTD.entry - 1
        )
        ...
        rightLines += [" " + repeat_right] * (
            (itemTD.height - itemTD.exit) + repeatTD.exit - 1
        )
...
class Group(DiagramItem):
    ...
    def textDiagram(self) -> TextDiagram:
        ...
        if self.label:
            ...
            diagramTD = labelTD.appendBelow(
                diagramTD, [], moveEntry=True, moveExit=True
            ).expand(0, 0, 1, 0)
        return diagramTD
...
class TextDiagram:
    ...
    def alter(
        self, entry: int = None, exit: int = None, lines: List[str] = None
    ) -> TextDiagram:
        """
    ...
    def appendBelow(
        ...
        newEntry = (
            self.height + len(linesBetween) + item.entry if moveEntry else self.entry
        )
    ...
    def center(self, width: int, pad: str) -> TextDiagram:
        ...
        if width == self.width:
            ...
            return self.__class__(
                self.entry,
                self.exit,
                TextDiagram._encloseLines(self.lines, left, right),
            )
    ...
    def expand(self, left: int, right: int, top: int, bottom: int) -> TextDiagram:
        ...
        if left + right + top + bottom == 0:
            ...
            for i in range(0, self.height):
                ...
                newLines += [
                    (leftExpansion * left) + self.lines[i] + (rightExpansion * right)
                ]
    ...
    @classmethod
    def setFormatting(
        cls, characters: Dict[str, str] = None, defaults: Dict[str, str] = None
    ) -> None:
        """
    ...
    def _rectish(cls, rect_type: str, data: TextDiagram, dashed=False) -> TextDiagram:
        ...
        (
            ...
            [
                ...
                f"{rect_type}_left{lineType}",
                f"{rect_type}_bot_left",
                f"{rect_type}_top_right",
                f"{rect_type}_right{lineType}",
                f"{rect_type}_bot_right",
                f"{rect_type}_top{lineType}",
                f"{rect_type}_bot{lineType}",
                "line",
                "cross",
            ]
        )
